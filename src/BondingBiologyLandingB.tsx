...
/**
 * BondingBiologyLandingB.tsx
 * ------------------------------------------------------------------
 * "/b" A/B variant of BondingBiologyLanding.tsx, promoting the private
 * Love Success Assessment / specialist evaluation offer instead of the
 * free blueprint. Intentionally a full duplicate (not a shared
 * component) so the control page at "/" is never at risk of changing.
 *
...
export default function BondingBiologyLandingB() {
  useReveal();
  const navigate = useNavigate();

  const goToApply = useCallback(
    (source: string) => navigate(`/b/offer?variant=b&source=${source}`),
    [navigate]
  );
  ...
  useEffect(() => {
    const handleMouseLeave = (e: MouseEvent) => {
      if (e.clientY < 20) {
        const hasShown = sessionStorage.getItem("exit_intent_shown_b");
        if (!hasShown) {
          sessionStorage.setItem("exit_intent_shown_b", "true");
          navigate("/b/offer?variant=b&source=exit_intent");
        }
      }
    };
