 ...
 * `?amount=&email=&first_name=`, those are read and used; otherwise this
 * falls back to the fixed $11 price.
 *
 * TODO: this page does not currently notify GHL of the purchase — the FB
 * pixel fire was the only requirement given for this page (same as
 * BondingBiologySummitThankYou.tsx). If the business also wants the sale
 * logged to GHL, that needs either a webhook call from here (if the
 * checkout passes back enough contact info) or a native GHL/Zapier
 * integration configured directly on the checkout provider's side.
 * ------------------------------------------------------------------
 */
...
const PURCHASE_TRACKED_KEY = "bb_e_purchase_tracked";
...
const NEXT_STEPS = [
  "Check your email for your receipt and access to the workbook, training, and audio downloads — it may take a few minutes to arrive.",
  "Add our email address to your contacts so nothing lands in spam.",
  "Everything is instant digital access, so you can start whenever it works for you.",
];

export default function BondingBiologyCheatCodeThankYou() {
  ...
  useEffect(() => {
    ...
    const value = Number.isFinite(amountParam) && amountParam > 0 ? amountParam : CHEAT_CODE_PRICE;
  ...
  return (
    <div className="bb-thank-you ff-sans relative min-h-screen overflow-x-clip bg-[#170006] text-[#FFF7EE] antialiased">
      ...
      <div className="absolute inset-0 bg-[url('/shay-stage.jpg')] bg-cover bg-[position:center_20%] bg-no-repeat opacity-[0.18] pointer-events-none" aria-hidden="true" />
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center px-5 py-16 text-center sm:px-8">
        ...
        <p className="mt-4 max-w-lg text-[16px] leading-[1.6] text-[#FFF7EE]/75">
          Your access to the High Value Woman Cheat Code is confirmed. Welcome, we're glad you're here.
        </p>
